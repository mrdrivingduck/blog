...
<span class="token punctuation">{</span>
    ...
    <span class="token comment">// ...</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>进入 <code>do_analyze_rel()</code> 函数后，内核代码将进一步明确要分析一个表中的哪些列：用户可能指定只分析表中的某几个列——被频繁访问的列才更有被分析的价值。然后还要打开待分析表的所有索引，看看是否有可以被分析的列。</p><p>为了得到每一列的统计信息，显然我们需要把每一列的数据从磁盘上读起来再去做计算。这里就有一个比较关键的问题了：到底扫描多少行数据呢？理论上，分析尽可能多的数据，最好是全部的数据，肯定能够得到最精确的统计数据；但是对一张很大的表来说，我们没有办法在内存中放下所有的数据，并且分析的阻塞时间也是不可接受的。所以用户可以指定要采样的最大行数，从而在运行开销和统计信息准确性上达成妥协：</p>`,55),d={href:"https://www.postgresql.org/docs/current/runtime-config-query.html#GUC-DEFAULT-STATISTICS-TARGET",target:"_blank",rel:"noopener noreferrer"},u=n("code",null,"default_statistics_target",-1),v=n("code",null,"ANALYZE",-1),m={href:"https://www.postgresql.org/docs/current/sql-altertable.html",target:"_blank",rel:"noopener noreferrer"},k=n("code",null,"ALTER TABLE ... ALTER COLUMN ... SET STATISTICS",-1),b=t(`<div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token comment">/*
...
<span class="token punctuation">{</span>
    ...
    <span class="token comment">// ...</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这个条件判断语句可以被解读为：</p><ul><li>如果说一个列的数据类型支持默认的 <code>=</code>（<code>eqopr</code>：equals operator）和 <code>&lt;</code>（<code>ltopr</code>：less than operator），那么这个列应该是一个数值类型，可以使用 <code>compute_scalar_stats()</code> 函数进行分析</li><li>如果列的数据类型只支持 <code>=</code> 运算符，那么依旧还可以使用 <code>compute_distinct_stats</code> 进行唯一值的统计分析</li><li>如果都不行，那么这个列只能使用 <code>compute_trivial_stats</code> 进行一些简单的分析</li></ul><p>我们可以分别看看这三个分析函数里做了啥，但我不准备深入每一个分析函数解读其中的逻辑了。因为其中的思想基于一些很古早的统计学论文，古早到连 PDF 上的字母都快看不清了...😱 咱数学不好，偷个懒。在代码上没有特别大的可读性，因为基本是参照论文中的公式实现的，不看论文根本没法理解变量和公式的含义。</p><h3 id="compute-trivial-stats" tabindex="-1"><a class="header-anchor" href="#compute-trivial-stats" aria-hidden="true">#</a> compute_trivial_stats</h3><p>如果某个列的数据类型不支持等值运算符和比较运算符，那么就只能进行一些简单的分析，比如：</p><ul><li>非空行的比例</li><li>列中元组的平均宽度</li></ul><p>这些可以通过对采样后的元组数组进行循环遍历后轻松得到。</p><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token comment">/*
...
<span class="token function">compute_distinct_stats</span><span class="token punctuation">(</span>VacAttrStatsP stats<span class="token punctuation">,</span>
                       ...
                       <span class="token keyword">double</span> totalrows<span class="token punctuation">)</span>
<span class="token punctuation">{</span><span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="summary" tabindex="-1"><a class="header-anchor" href="#summary" aria-hidden="true">#</a> Summary</h2><p>以 PostgreSQL 优化器需要的统计信息为切入点，分析了 <code>ANALYZE</code> 命令的大致执行流程。出于简洁性，在流程分析上没有覆盖各种 corner case 和相关的处理逻辑。同时避开了所有和数学相关的细节 🤪。</p><h2 id="references" tabindex="-1"><a class="header-anchor" href="#references" aria-hidden="true">#</a> References</h2>`,30),h={href:"https://www.postgresql.org/docs/current/sql-analyze.html",target:"_blank",rel:"noopener noreferrer"},g={href:"https://www.postgresql.org/docs/current/routine-vacuuming.html#VACUUM-FOR-STATISTICS",target:"_blank",rel:"noopener noreferrer"},f={href:"https://www.postgresql.org/docs/current/planner-stats.html",target:"_blank",rel:"noopener noreferrer"},w={href:"https://www.postgresql.org/docs/current/catalog-pg-statistic.html",target:"_blank",rel:"noopener noreferrer"},y={href:"http://mysql.taobao.org/monthly/2016/05/09/",target:"_blank",rel:"noopener noreferrer"};function _(T,A){const a=i("ExternalLinkIcon");return l(),p("div",null,[r,n("blockquote",null,[n("p",null,[s("通过设置 GUC 参数 "),n("a",d,[u,e(a)]),s(" 为更大的值，可以全局增大采样的数据量。这会导致 "),v,s(" 所需要的整体时间变长，但可以得到更准确的统计信息（尤其是对大表）。如果全局设置带来的影响较大，可以通过 "),n("a",m,[k,e(a)]),s(" 为某一个列单独设置采样数据量。")])]),b,n("p",null,[n("a",h,[s("PostgreSQL 14 Documentation: ANALYZE"),e(a)])]),n("p",null,[n("a",g,[s("PostgreSQL 14 Documentation: 25.1. Routine Vacuuming"),e(a)])]),n("p",null,[n("a",f,[s("PostgreSQL 14 Documentation: 14.2. Statistics Used by the Planner"),e(a)])]),n("p",null,[n("a",w,[s("PostgreSQL 14 Documentation: 52.49. pg_statistic"),e(a)])]),n("p",null,[n("a",y,[s("阿里云数据库内核月报 2016/05：PostgreSQL 特性分析 统计信息计算方法"),e(a)])])])}const x=o(c,[["render",_],["__file","PostgreSQL Analyze.html.vue"]]);export{x as default};
