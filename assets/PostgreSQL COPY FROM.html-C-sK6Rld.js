import{_ as n,c as s,o as a,a as e}from"./app-zsROWd52.js";const p={},t=e(`<h1 id="postgresql-copy-from" tabindex="-1"><a class="header-anchor" href="#postgresql-copy-from"><span>PostgreSQL - COPY FROM</span></a></h1><p>Created by: Mr Dk.</p><p>2023 / 09 / 28 00:04</p><p>Qingdao, Shandong, China</p><hr><h2 id="background" tabindex="-1"><a class="header-anchor" href="#background"><span>Background</span></a></h2><p>PostgreSQL 的 <code>COPY FROM</code> 语法用于将来自外部 <em>文件</em>（磁盘文件 / 网络管道 / IPC 管道）的数据导入到数据库的表中。<code>COPY FROM</code> 支持只导入指定的部分列，其它列被填充为默认值。<code>COPY FROM</code> 还支持带有 <code>WHERE</code> 子句，只允许满足条件的行被导入到表中。</p><p><code>COPY FROM</code> 的实现逻辑比 <code>COPY TO</code> 相对复杂一些。其原因在于，<code>COPY TO</code> 是要把数据库中的数据导出到外部，其中获取数据这一步及其并行优化，很大程度上借助了优化器和执行器的能力，复用了很多代码；而 <code>COPY FROM</code> 是要把外部数据导入数据库，其中写入数据库的行为因需要更高效的定制实现，而不能复用 <code>INSERT</code> 相关的执行器代码了。</p><p>本文基于当前 PostgreSQL 主干开发分支（PostgreSQL 17 under devel）源代码对这个过程进行分析。分析过程中发现 <code>COPY FROM</code> 的代码存在一些小小的问题（真的是很小的问题..），于是误打误撞地向 PostgreSQL 社区贡献了自己的第一个 patch：</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre class="language-text"><code><span class="line">commit e434e21e114b423e919324ad6ce1f3f079ca2a03</span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果指定了 <code>WHERE</code> 子句，那么还需要将其处理为布尔表达式：</p><div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c" data-title="c"><pre class="language-c"><code><span class="line"><span class="token keyword">if</span> <span class="token punctuation">(</span>stmt<span class="token operator">-&gt;</span>whereClause<span class="token punctuation">)</span></span>
<span class="line"><span class="token punctuation">{</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>接下来，执行器逻辑开始处理 <code>COPY FROM</code> 的具体事宜。与 <code>COPY TO</code> 类似，<code>BeginCopyFrom</code> / <code>CopyFrom</code> / <code>EndCopyFrom</code> 三个函数分别对应了三个执行阶段：</p><ol><li>准备</li><li>执行</li><li>结束</li></ol><div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c" data-title="c"><pre class="language-c"><code><span class="line"><span class="token keyword">if</span> <span class="token punctuation">(</span>is_from<span class="token punctuation">)</span></span>
<span class="line"><span class="token punctuation">{</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="初始化过滤条件" tabindex="-1"><a class="header-anchor" href="#初始化过滤条件"><span>初始化过滤条件</span></a></h3><p>如果 <code>COPY FROM</code> 指定了 <code>WHERE</code> 子句，那么初始化过滤条件：</p><div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c" data-title="c"><pre class="language-c"><code><span class="line"><span class="token keyword">if</span> <span class="token punctuation">(</span>cstate<span class="token operator">-&gt;</span>whereClause<span class="token punctuation">)</span></span>
...
<span class="line">                                             resultRelInfo<span class="token punctuation">)</span><span class="token punctuation">;</span></span>
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>从输入文件描述符中获取一行数据，根据其格式，通过初始化阶段中准备好的转换函数和默认值表达式，转换为数据库内部的元组表示形式（此处省略内部细节），并存储在槽位中：</p><div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c" data-title="c"><pre class="language-c"><code><span class="line"><span class="token comment">/* Directly store the values/nulls array in the slot */</span></span>
