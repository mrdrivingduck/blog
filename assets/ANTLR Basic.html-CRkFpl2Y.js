import{_ as n,c as a,a as e,o as l}from"./app-DfFNXaa-.js";const p="/blog/assets/antlr-gen-kFD5D7XW.png",i="/blog/assets/antlr-test-rule-DpsO8EHy.png",c="/blog/assets/antlr-preview-DT28kA48.png",t={};function o(r,s){return l(),a("div",null,[...s[0]||(s[0]=[e(`<h1 id="antlr-basic" tabindex="-1"><a class="header-anchor" href="#antlr-basic"><span>ANTLR - Basic</span></a></h1><p>Created by : Mr Dk.</p><p>2020 / 01 / 14 17:25</p><p>Nanjing, Jiangsu, China</p><hr><p><strong>ANTLR (ANother Tool for Language Recognition)</strong> 是一套计算机语言处理框架，可用于对特定语法的语言进行词法分析、语法分析、语法树构建。最新的 ANTLR v4 规定了一套 <strong>g4</strong> 语法。g4 语法可以针对对象语言进行类似 <em>巴克斯范式 (Backus-Naur From, BNF)</em> 的描述。ANTLR 可以将 g4 语法文件直接转换为词法分析和语法分析的 Java 代码 (或其它实现语言)，这样就可以得到一个 Java 实现的语言识别器，比如：</p><ul><li>SQL 识别器</li><li>JSON 识别器</li></ul><p>除了用于自动生成 Java 代码，ANTLR 还提供一个 <a href="https://github.com/antlr/antlr4" target="_blank" rel="noopener noreferrer">Runtime 工具</a>，用于为自动生成的 Java 代码的执行提供支持。Runtime 需要作为依赖 (如 Maven) 引入。对于流行通用的编程语言、格式，开源社区都已经提供了 <a href="https://github.com/antlr/grammars-v4" target="_blank" rel="noopener noreferrer">现成的 g4 语法文件</a> 🤞</p><p>本文以生成一个 Java 版本的 JSON 解析器作为 🌰。</p><h2 id="example" tabindex="-1"><a class="header-anchor" href="#example"><span>Example</span></a></h2><p>以下是 <em>grammars-v4</em> 中提供的 JSON 语法文件 <code>JSON.g4</code></p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre><code class="language-text"><span class="line">/** Taken from &quot;The Definitive ANTLR 4 Reference&quot; by Terence Parr */</span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>运行该程序，可以看到指定的输入是否可以被该识别器所接受。</p><p>此外，插件还带有语法树预览的功能，在语法文件的 <em>top-level rule (顶层语法规则)</em> 右击 <code>Test Rule &lt;rule_name&gt;</code>：</p><img src="`+i+'" alt="antlr-test-rule" style="zoom:50%;"><p>从文本框或文件接收测试输入后，就能看到预览的 AST：</p><p><img src="'+c+`" alt="antlr-preview"></p><h2 id="listener-visitor" tabindex="-1"><a class="header-anchor" href="#listener-visitor"><span>Listener &amp;&amp; Visitor</span></a></h2><p>使用 ANTLR 框架的更多需求是，当进入或退出某条语法规则的时候，我们可能想做一些事情，比如在 AST 上做一定的转换。ANTLR 允许我们在语法树上注册回调函数，并提供了两种遍历方式 - listener 和 visitor。</p><h3 id="listener" tabindex="-1"><a class="header-anchor" href="#listener"><span>Listener</span></a></h3><p>使用 ANTLR 的代码生成工具，得到了一个 <code>JSONListener.java</code>。这是一个接口文件，里面定义了 <strong>进入</strong> 和 <strong>退出</strong> 每一条语法规则的函数：</p><div class="language-java line-numbers-mode" data-highlighter="prismjs" data-ext="java"><pre><code class="language-java"><span class="line"><span class="token keyword">public</span> <span class="token keyword">interface</span> <span class="token class-name">JSONListener</span> <span class="token keyword">extends</span> <span class="token class-name">ParseTreeListener</span> <span class="token punctuation">{</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="label" tabindex="-1"><a class="header-anchor" href="#label"><span>Label</span></a></h3><p>对于某一条语法规则，如果想生成更加细粒度的函数进行分别处理该怎么做？</p><p>比如，对于 JSON 中 value 的取值：</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text"><pre><code class="language-text"><span class="line">value</span>
<span class="line">   : STRING</span>
<span class="line">   | NUMBER</span>
<span class="line">   | obj</span>
<span class="line">   | array</span>
