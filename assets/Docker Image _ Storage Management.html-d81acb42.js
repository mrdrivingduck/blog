import{_ as l,r as n,o as c,c as a,a as e,b as o,d,e as r}from"./app-25fa875f.js";const t="/blog/assets/union-mount-407ef8e3.png",s={},u=r('<h1 id="docker-image-storage-management" tabindex="-1"><a class="header-anchor" href="#docker-image-storage-management" aria-hidden="true">#</a> Docker - Image &amp; Storage Management</h1><p>Created by : Mr Dk.</p><p>2020 / 09 / 21 17:46</p><p>Nanjing, Jiangsu, China</p><hr><h2 id="docker-镜像管理" tabindex="-1"><a class="header-anchor" href="#docker-镜像管理" aria-hidden="true">#</a> Docker 镜像管理</h2><p>Docker 镜像是一个只读版的 Docker 容器模板，含有启动 Docker 容器所需的文件结构和内容，是 Docker 容器的静态视角。</p><h3 id="rootfs" tabindex="-1"><a class="header-anchor" href="#rootfs" aria-hidden="true">#</a> rootfs</h3><p>rootfs 是 Docker 容器启动时，内部进程可见的文件系统，也就是 Docker 容器的根目录。通常包含一个 OS 运行所需的文件系统，比如 <code>/bin</code>、<code>/lib</code>、<code>/usr</code> 等。</p><p>传统 Linux 内核启动时，会挂载一个只读的 rootfs，在系统检测其完整性后，再将其切换为可读写模式。在 Docker 中，Docker daemon 为 Docker 容器挂载 rootfs 时，也会将其设置为只读模式。挂载完毕后，利用 <em>联合挂载 (union mount)</em> 技术在 rootfs 的挂载点上再挂载一个空的读写文件系统。当 Docker 容器运行过程中，文件系统发生变化时，发生变化的内容将会写到读写文件系统，并隐藏只读文件系统中的旧版本文件。</p><h3 id="docker-镜像的特点" tabindex="-1"><a class="header-anchor" href="#docker-镜像的特点" aria-hidden="true">#</a> Docker 镜像的特点</h3><ul><li>分层 <ul><li>修改容器中的文件时，只对最上层读写文件系统进行变动，不覆盖下层已有文件系统的内容</li><li><code>docker commit</code> 提交修改后的文件系统时，只保存读写文件系统中的被更新过的文件</li></ul></li><li>写时复制 <ul><li>只有对只读文件系统中的文件修改时，才将其复制到读写文件系统中，并应用修改</li></ul></li><li>内容寻址 <ul><li>对镜像层的内容计算校验和，生成一个内容 hash 作为镜像层 (文件系统) 的唯一标志</li></ul></li><li>联合挂载 <ul><li>联合挂载技术可以在 <strong>一个挂载点</strong> 同时挂载多个文件系统，多个文件系统中的目录将被整合</li></ul></li></ul><img src="'+t+`" alt="union-mount" style="zoom:50%;"><h3 id="镜像构建过程" tabindex="-1"><a class="header-anchor" href="#镜像构建过程" aria-hidden="true">#</a> 镜像构建过程</h3><p>如果使用 <code>docker commit</code> 命令 - 只提交容器镜像发生了变更的部分 (即读写文件系统)：</p><ol><li>确定是否暂停容器运行</li><li>将容器的 <strong>可读写层</strong> 打包</li><li>在层存储中注册打包后的可读写层 (也就是带有差异的文件集合)</li><li>更新镜像历史信息，在镜像存储中创建新的镜像，记录元数据</li><li>给镜像添加 tag 信息 (如果指定)</li></ol><p>如果使用 Dockerfile + <code>docker build</code> 命令构建镜像：</p><ol><li>Docker client 在本地准备好构建上下文，然后将上下文信息发送到 Docker server</li><li>Docker server 创建一个临时目录，将上下文中的文件系统解压到这个目录下</li><li>读取并解析 Dockerfile，遍历指令，分发到不同的模块执行</li><li>为每条指令生成一个临时容器，执行一条指令，就使用 <code>commit</code> 生成一个镜像层</li><li>Dockerfile 中所有指令对应的层的集合，就是 build 的结果</li><li>如果指定了 tag 参数，那么给镜像打上对应的 tag</li></ol><hr><h2 id="docker-存储管理" tabindex="-1"><a class="header-anchor" href="#docker-存储管理" aria-hidden="true">#</a> Docker 存储管理</h2><h3 id="docker-镜像元数据管理" tabindex="-1"><a class="header-anchor" href="#docker-镜像元数据管理" aria-hidden="true">#</a> Docker 镜像元数据管理</h3><p>Docker 镜像在设计上将 <strong>元数据</strong> 与 <strong>文件</strong> 的存储完全隔离。</p><h4 id="repository-元数据" tabindex="-1"><a class="header-anchor" href="#repository-元数据" aria-hidden="true">#</a> Repository 元数据</h4><p>Repository 是具有某个功能的 Docker 镜像的 <strong>所有迭代版本</strong> 构成的镜像库。 其元数据包含所有 repository 的名字及其名下所有版本镜像的名字 + tag (比如 <code>ubuntu:20.04</code>)，以及该版本镜像相应的镜像 ID (目前 Docker 使用 SHA256 来计算镜像 ID)。这些元数据用于管理 repository 与镜像 ID 的映射关系。</p><div class="language-console line-numbers-mode" data-ext="console"><pre class="language-console"><code>$ sudo cat /var/lib/docker/image/overlay2/repositories.json | python -mjson.tool
{
    &quot;Repositories&quot;: {
        &quot;hello-world&quot;: {
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="image-元数据" tabindex="-1"><a class="header-anchor" href="#image-元数据" aria-hidden="true">#</a> Image 元数据</h4><p>其中包含了镜像的一些元信息，如：</p><ul><li>镜像架构</li><li>OS</li><li>镜像默认配置</li><li>容器 ID 和配置</li><li>创建时间</li><li>Docker 版本</li><li>镜像构建历史</li><li>rootfs</li></ul><p>将镜像与构建镜像的镜像层关联了起来。Docker 会根据 <code>diff_ids</code> 计算出镜像层的存储索引 chainID。</p><div class="language-console line-numbers-mode" data-ext="console"><pre class="language-console"><code>$ sudo cat /var/lib/docker/image/overlay2/imagedb/content/sha256/bf756fb1ae65adf866bd8c456593cd24beb6a0a061dedf42b26a993176745f6b | python -mjson.tool
{
    &quot;architecture&quot;: &quot;amd64&quot;,
    &quot;config&quot;: {
        &quot;ArgsEscaped&quot;: true,
