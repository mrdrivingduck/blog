import{_ as n,c as s,o as a,a as p}from"./app-DfdLUkTv.js";const e={},t=p(`<h1 id="chapter-13-1-13-5-邮件代理模块-认证服务器" tabindex="-1"><a class="header-anchor" href="#chapter-13-1-13-5-邮件代理模块-认证服务器"><span>Chapter 13.1-13.5 - 邮件代理模块 - 认证服务器</span></a></h1><p>Created by : Mr Dk.</p><p>2020 / 08 / 04 10:22</p><p>Nanjing, Jiangsu, China</p><hr><h2 id="_13-1-邮件代理服务器的功能" tabindex="-1"><a class="header-anchor" href="#_13-1-邮件代理服务器的功能"><span>13.1 邮件代理服务器的功能</span></a></h2><p>邮件代理服务器 <strong>不提供实际的邮件服务器功能</strong>，只负责把客户端的请求代理到上游的邮件服务器中。</p><ul><li>认证 - 只有经过认证服务器的认证后，Nginx 才会向上游邮件服务器发起通信请求</li><li>透传</li></ul><p>Nginx 与下游客户端、上游邮件服务器之间都是使用邮件协议，而与认证服务器之间使用的是类 HTTP 协议。Nginx 邮件模块的目的是与上游邮件服务器之间透传 TCP 流。在邮件模块的配置中，直属于 <code>mail{}</code> 块的配置称为 main 级别配置，直属于 <code>server{}</code> 块下的配置则被称为 srv 配置。</p><h2 id="_13-2-邮件模块的处理框架" tabindex="-1"><a class="header-anchor" href="#_13-2-邮件模块的处理框架"><span>13.2 邮件模块的处理框架</span></a></h2><p>与 HTTP 处理类似，Nginx 把邮件请求的处理过程分为八个阶段 (把相同代码可能会被多次调用的过程划分为一个阶段)：</p><ol><li>Nginx 与客户端建立 TCP 连接，回调 <code>ngx_mail_init_connection()</code> 初始化将要用到的数据结构</li><li>Nginx 接收、解析客户端请求</li><li>Nginx 与认证服务器建立 TCP 连接</li><li>Nginx 向认证服务器发起类 HTTP 请求</li><li>Nginx 接收认证服务器的响应并判断是否合法，并获得上游邮件服务器的地址</li><li>Nginx 向上游邮件服务器发起 TCP 连接</li><li>Nginx 与邮件服务器使用 POP3 / SMTP / IMAP 协议交互认证</li><li>Nginx 在客户端与邮件服务器之间透传协议</li></ol><p>邮件模块的定义与 HTTP 模块类似。首先，有一个 <code>ngx_mail_module</code> 的核心模块，在其中定义了新的模块类型 <code>NGX_MAIL_MODULE</code>。这类模块的 <code>ctx</code> 成员指向的通用抽象接口为 <code>ngx_mail_module_t</code>：</p><div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c" data-title="c"><pre><code><span class="line"><span class="token keyword">typedef</span> <span class="token keyword">struct</span> <span class="token punctuation">{</span></span>
