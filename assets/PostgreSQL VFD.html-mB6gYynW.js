import{_ as n,c as s,o as a,a as e}from"./app-zsROWd52.js";const p={},l=e(`<h1 id="postgresql-vfd" tabindex="-1"><a class="header-anchor" href="#postgresql-vfd"><span>PostgreSQL - VFD</span></a></h1><p>Created by : Mr Dk.</p><p>2022 / 08 / 08 0:27</p><p>Hangzhou, Zhejiang, China</p><hr><h2 id="background" tabindex="-1"><a class="header-anchor" href="#background"><span>Background</span></a></h2><p>PostgreSQL 后端进程经常需要打开大量的文件，包括表文件、索引文件、临时文件（用于排序或构造 hash 表）等。由于操作系统允许一个进程能够打开的文件数量是有上限的，为了防止后端进程打开文件时因超出 OS 的限制而失败，PostgreSQL 提供了 <strong>虚拟文件描述符 (VFD)</strong> 机制。VFD 抽象层能够对使用 VFD API 的代码屏蔽对 OS 文件描述符的管理细节，使更高层的代码能够打开比 OS 限制的数量更多的文件。</p><p>本文对 PostgreSQL 内核代码的分析截止主干开发分支 <code>master</code> 上的如下版本号：</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre class="language-text"><code><span class="line">commit afe58c8b746cac1e2c3e9f0fc96a0f69a46c84d3</span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>虚拟文件描述符的结构定义如下：</p><div class="language-c line-numbers-mode" data-highlighter="prismjs" data-ext="c" data-title="c"><pre class="language-c"><code><span class="line"><span class="token keyword">typedef</span> <span class="token keyword">struct</span> <span class="token class-name">vfd</span></span>
